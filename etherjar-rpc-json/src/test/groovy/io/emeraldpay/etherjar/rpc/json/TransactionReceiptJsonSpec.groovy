...
class TransactionReceiptJsonSpec extends Specification {
    ...
    def "Parse receipt 0xe9230e with many logs"() {
        InputStream json = TransactionReceiptJsonSpec.classLoader.getResourceAsStream("receipt/0xe9230e.json")

        when:
        def act = objectMapper.readValue(json, TransactionReceiptJson)

        then:
        act.blockHash.toHex() == "0x64186662801b9070433f9fbd488fa28afea5c25de660d9107801f8dd6f0e32d3"
        act.cumulativeGasUsed == 0xa38cf
        act.logs.size() == 12
        act.logsBloom.toHex().startsWith("0x10a05")
    }
}
